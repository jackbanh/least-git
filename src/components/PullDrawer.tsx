...
export default function PullDrawer({
  ...
  useEffect(() => {
    ...
    const unlistenLine = listen<{ tab_id: string; line: string }>("pull:line", (e) => {
      if (e.payload.tab_id !== tabId) return;
      setLines((prev) => [...prev, e.payload.line]);
    });

    const unlistenDone = listen<{ tab_id: string; success: boolean }>("pull:done", (e) => {
      if (e.payload.tab_id !== tabId) return;
      setStatus(e.payload.success ? "success" : "error");
      if (e.payload.success) onSuccess();
    });

    invoke("pull_with_rebase", { tabId }).catch((e: unknown) => {
